...
void GxEPD2_EPD::init(uint32_t serial_diag_bitrate, bool initial, uint16_t reset_duration, bool pulldown_rst_mode)
{
  ...
  if (_cs >= 0)
  {
    ...
    pinMode(_cs, OUTPUT);
    digitalWrite(_cs, HIGH); // set
  }

  _reset();
  _pSPIx->begin(); // Initialize SPI

  if (_rst >= 0)
  {
    digitalWrite(_rst, HIGH); // preset
    pinMode(_rst, OUTPUT);
    digitalWrite(_rst, HIGH); // set
  }

  if (_cs >= 0)
  {
    digitalWrite(_cs, HIGH); // preset
    pinMode(_cs, OUTPUT);
    digitalWrite(_cs, HIGH); // set
  }
  ...
  if (_dc >= 0)
  {
    digitalWrite(_dc, HIGH); // preset
    pinMode(_dc, OUTPUT);
    digitalWrite(_dc, HIGH); // set
  }
...
void GxEPD2_EPD::_reset()
{
  if (_rst >= 0)
  {
    ...
    {
      digitalWrite(_rst, HIGH); // NEEDED for Waveshare "clever" reset circuit
      pinMode(_rst, OUTPUT);
      digitalWrite(_rst, HIGH);
      delay(10); // at least delay(2)
      digitalWrite(_rst, LOW);
      delay(_reset_duration);
      digitalWrite(_rst, HIGH);
      delay(_reset_duration > 10 ? _reset_duration : 10);
